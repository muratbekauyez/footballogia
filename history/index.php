...
<body>
    <div id="sticky">
        ...
        <nav>
            <!-- the navigation bar -->
            <ul>
                <li><a href="../history" class="thisPage">История Футбола</a></li>
    ...
    <section id="content">
        ...
        if (isset($_POST["updateHistory"])) {
            ...
            $sql = "UPDATE one_pagers SET content_page = '$content_page' WHERE id_page = 1";
            $con->query($sql);
        }
        //showing the page editor
        if (isset($_POST["editHistory"])) {
            $pageDetails = mysqli_fetch_assoc (mysqli_query ($con, "SELECT * FROM one_pagers WHERE id_page = 1"));
            ?>
            <form action='' method='post' target='_self'>
                <fieldset>
                    ...
                    <br>
                    <input type='submit' name='updateHistory' class='submit' value='Изменить'>
                </fieldset>
            </form>
            <div id='editorAddon'>
        ...
        if ($_SESSION["logged_in"] == true) {
            ...
            <fieldset>
                <form action='' method='post' target='_self'>
                    <label for='editHistory'>Изменить Данные Страницы</label>
                    <br>
                    <input type='submit' name='editHistory' class='submit' value='Изменить'>
                </form>
            </fieldset>
        <?php
        }
        //outputing the contents of the page that are in the database on the webpage
        $history = mysqli_fetch_assoc (mysqli_query ($con, "SELECT * FROM one_pagers WHERE id_page = 1"));
        echo "<p id='blogText'>" . html_entity_decode($history["content_page"], ENT_QUOTES | ENT_HTML401) . "</p>";
        ?>
    </section>
</body>
</html>